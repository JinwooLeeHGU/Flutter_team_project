...
class _HomePageState extends State<HomePage> {
  ...
  Widget _buildBody(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final collection = FirebaseFirestore.instance.collection('products').orderBy("moddate", descending: true);
        return StreamBuilder<QuerySnapshot>(
          stream: collection.where("address", isEqualTo: currUser.address).snapshots(),
          builder: (context, snapshot) {
            if (!snapshot.hasData) return LinearProgressIndicator();
  ...
  Widget _buildGridCards(BuildContext context, DocumentSnapshot data) {
    ...
    if(calculateDueDate(record.deadline) == "마감" || record.accept == record.buys) {
